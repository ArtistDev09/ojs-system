			...
			<div class="container">
				<div class="row">
					<nav aria-label="{translate|escape key="common.navigation.user"}">
                     {* --- Logo --- *}
    <a href="{$homeUrl}" class="navbar-logo" style="display: inline-flex; align-items: center; margin-right: 3px; padding-top:3px;">
...
<div class="rdd-header"
    ...
    <h1 class="journal_title" style="color: white; font-size: 30px">
        ...
        {elseif $displayPageHeaderTitle && is_array($displayPageHeaderTitle)}
            <a href="{$homeUrl}" class="navbar-brand navbar-brand-logo" style="text-decoration: none; color: inherit;">
                {$pageTitleTranslated}
            </a>
        {else}
            <a href="{$homeUrl}" class="navbar-brand" style="text-decoration: none; color: inherit;">
                OJS Site Name
            </a>
        {/if}
    </h1>
